...
def dirunal_inequality(
        m2_g: ArrayLike,
        o1_g: ArrayLike,
        k1_g: ArrayLike) -> ArrayLike:
    """
    Compute diurnal inequality based on Balay, 1952.
    ...
    :return: Diurnal inequality defined by Balay, 1952.
    """
...
def grid_around(
    ...
    """
    Returns the surrounding grid nodes.

    Finds the surrounding nodes from the structured grid defined by `grid_lon` and `grid_lat` at the location defined
    ...
    if ieast == 0:
        ...
        iwest = ieast - 1
    surrounding_lon = grid_x[[iwest, ieast]]
    ...
    surrounding_lat = grid_y[[isouth, inorth]]

    # returning surrounding nodes in counter-clockwise order
    surrounding_nodes = np.array([
        [surrounding_lon[0], surrounding_lat[0]],  # [west, south]
        [surrounding_lon[1], surrounding_lat[0]],  # [east, south]
        [surrounding_lon[1], surrounding_lat[1]],  # [east, north]
        [surrounding_lon[0], surrounding_lat[1]]  # [west, north]
    ])
...
def tri_around(
    ...
    """
    Finds the surrounding nodes in a triangular mesh.
